...
版本異動紀錄 (Version History):
    v1.9.3 (2026-03-31)
        - 效能優化：使用 `xxhash.xxh64` 取代 `md5` 進行檔案內容雜湊比對，大幅提升 TB 級整理速度。
    v1.9.2 (2026-03-02)
        - 實作 Requirement 20：新增 `--delete-json` 參數，明確控制 Takeout 模式下 JSON 的刪除行為。
    v1.9.1 (2026-03-02)
        - 實作 Requirement 19：掃描時主動忽略副檔名為 `.delete` 的檔案。
...
try:
    ...
    print(f"CRITICAL ERROR: 缺少必要套件 ({e.name})。請執行: pip install loguru Pillow xxhash")
    sys.exit(1)
...
class FileOrganizer:
    ...
    def _determine_date(self, filepath: Path) -> Tuple[Optional[datetime], str, Optional[Path]]:
        ...
        if self.date_discovery_strategies == ['takeout']:
            ...
            # 擴充支援 (n) 變體，例如 .supplemental-metadata(1).json (Req 20)
            for i in range(1, 10):
                json_candidates.append(filepath.parent / f"{filepath.name}.supplemental-metadata({i}).json")
                json_candidates.append(filepath.parent / f"{filepath.name}({i}).json")
    ...
    def _execute_operation(self, src: Path, dst: Path, dt: datetime, used_json: Optional[Path]) -> bool:
        ...
        try:
            ...
            if self.operation_mode == 'copy':
                ...
                # 搬移模式下且啟用 delete-json，一併刪除已使用的 JSON 檔案 (Req 20)
                if self.delete_json and used_json and used_json.exists():
                    try:
                        os.remove(used_json)
            ...
            if self._are_files_identical(src_path, dest_path):
                if self.operation_mode == 'move' or self.delete_duplicate_source:
                    ...
                    if not self.dry_run:
                        try:
                            os.remove(src_path)
                            # 來源檔案被判定為重複且遭刪除時，若啟用 delete-json 則一併清除 JSON (Req 20)
                            if self.delete_json and used_json and used_json.exists():
                                os.remove(used_json)
        ...
        try:
            for src_root in self.sources:
                ...
                for root, dirs, files in os.walk(src_root):
                    ...
                    for name in files:
                        # 忽略副檔名為 .delete 的檔案 (Req 19)
                        if name.lower().endswith('.delete'):
                            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="照片整理工具 (v1.9.3 企業最終版)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="使用範例:\n  python3 file_organizer.py -t /dest /src1 --workers 8 --move --mode takeout --delete-json"
    )
    ...
    parser.add_argument('--delete-json', action='store_true', help='Takeout 模式下實際刪除關聯的 JSON 檔案')
